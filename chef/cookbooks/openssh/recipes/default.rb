...
if node['openssh']['disable_root_opts']
  execute "cp /root/.ssh/authorized_keys /root/.ssh/authorized_keys.disable_root_opts" do
    cwd '/root/.ssh/'
  end

  execute "sed -i \"s/'//g\" authorized_keys" do
    cwd '/root/.ssh/'
  end
  ...
  execute 'sed -i "s/no-port-forwarding,no-agent-forwarding,no-X11-forwarding,command=echo Please login as the user ubuntu rather than the user root.;echo;sleep 10 //g" authorized_keys' do
    cwd '/root/.ssh/'
  end
else 
  execute "mv /root/.ssh/authorized_keys.disable_root_opts /root/.ssh/authorized_keys" do
    cwd '/root/.ssh/'
    only_if { ::File.exists?("/root/.ssh/authorized_keys.disable_root_opts")}
  end
end
